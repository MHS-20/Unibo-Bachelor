...
int main(int argc, char **argv)
{
    ...
    while (gets(req.file_name))
    {
        /* invio richiesta */
        len = sizeof(servaddr);
        printf("Parola da eliminare nel file: ");
        gets(req.word);
        printf("Parola inserita: %s", req.word);

        // problema: viene inviato solo il nome file ma non la parola
        ...
        if (recvfrom(sd, &num_deleted, sizeof(num_deleted), 0, (struct sockaddr *)&servaddr, &len) < 0)
        {
            perror("recvfrom");
        ...
        if (num_deleted < 0)
            ...
            printf("Nel file %s sono state eliminate %d parole\n", req.file_name, num_deleted);
        printf("Nome del file: ");
