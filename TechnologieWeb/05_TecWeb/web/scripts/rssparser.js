...
function parsificaXml(xml) {
	...
	var respText = JSON.parse(items); //array

	var risultato = "<ul>";
	for (var i = 0; c < respText.length; i++) {
		risultato += '<li class="item"><strong>' + respText[i].title + '</strong><br/>';
		risultato += respText[i].description + "<br/>";
		risultato += '<a href="' + respText[i].link + '">approfondisci</a><br/></li>';
	};
	risultato += "</ul>";
	return risultato; 
 ...
 */
function caricaFeed(uri, e) { //uri, element
