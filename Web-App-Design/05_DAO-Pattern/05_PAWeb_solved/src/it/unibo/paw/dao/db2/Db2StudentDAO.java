...
public class Db2StudentDAO implements StudentDAO {
	...
	public void create(StudentDTO t) {
		...
		try {
			...
			/*
			//ON CASCADE
			//in questa parte commentata si danno gli strumenti per utilizzare anche la tecnica di on cascade
			//per ogni corso seguito dallo studente si va (attraverso la classe Db2StudentCourseMapping) a inserire le tuple di associazione nella tabella di mapping
			Db2StudentCourseMappingDAO csmdao = new Db2StudentCourseMappingDAO();
			for(CourseDTO course: t.getCourses())
				csmdao.create(course.getId(), t.getId());
			*/
		}
		// --- 5. Gestione di eventuali eccezioni ---
		catch (Exception e) {
