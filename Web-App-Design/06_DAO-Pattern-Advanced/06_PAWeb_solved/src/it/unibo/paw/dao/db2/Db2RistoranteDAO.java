...
public class Db2RistoranteDAO implements RistoranteDAO {
	...
	public void create(RistoranteDTO ristorante) {
		...
		try {
			//SOLUZIONE CON SEQUENCE
			// --- a. Richiesta all'id broker per un id valido da assegnare all'oggetto 
			IdBroker broker = new Db2IdBroker();
			ristorante.setId(broker.newId());
			
			// --- b. Crea (se senza parametri) o prepara (se con parametri) lo statement
			PreparedStatement prep_stmt = conn.prepareStatement(Db2RistoranteDAO.insert);
			// --- c. Pulisci e imposta i parametri (se ve ne sono)
			...
			// --- d. Esegui l'azione sul database ed estrai il risultato (se atteso)
			prep_stmt.executeUpdate();
			// --- e. Cicla sul risultato (se presente) pe accedere ai valori di ogni sua tupla
			// n.d.
			// --- f. Rilascia la struttura dati del risultato
			// n.d.
			// --- g. Rilascia la struttura dati dello statement
			prep_stmt.close();
		}
